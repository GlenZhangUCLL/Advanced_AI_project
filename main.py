...
def query_llm_executive_summary(gap_report_body, selected_name):
    """
    Generates a professional, high-level compliance summary based on 
    the verified structural markdown report generated by Python.
    """
    prompt = f"""
You are an expert Retail Space Automation AI compliance auditor. 
Review the following verified planogram variance report generated by our vision pipeline for "{selected_name}":

{gap_report_body}

Write a concise, professional two-sentence 'Executive Summary Statement of Compliance' evaluating these discrepancies. Do not list individual item IDs again.
"""
    ...
    try:
        response = ollama.chat(
            model='phi3', 
            messages=[
                {"role": "system", "content": "You are a senior retail compliance auditor. Write only a brief, high-level executive summary statement based on the provided data."},
                {"role": "user", "content": prompt}
            ],
            options={"temperature": 0.0} # Maintain absolute determinism
        )
